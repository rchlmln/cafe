...
class Laporan extends MY_Controller 
{
	...
	public function excel($from, $to)
	{
		...
		{
			$filename = 'Report_Sales_'.$from.'_'.$to;
			header("Content-type: application/x-msdownload");
			header("Content-Disposition: attachment; filename=".$filename.".xls");

			echo "
				<h4>Sales Report Date ".date('d/m/Y', strtotime($from))." - ".date('d/m/Y', strtotime($to))."</h4>
	...
	public function pdf($from, $to)
	{
		...
		$pdf->Cell(0, 8, "Sales Report Date ".date('d/m/Y', strtotime($from))." - ".date('d/m/Y', strtotime($to)), 0, 1, 'L'); 

		$pdf->Cell(15, 7, 'No', 1, 0, 'L'); 
		$pdf->Cell(85, 7, 'Date', 1, 0, 'L');
		$pdf->Cell(85, 7, 'Total Sales', 1, 0, 'L'); 
		$pdf->Ln();
		...
		$pdf->Cell(100, 7, 'Total All Sales', 1, 0, 'L'); 
